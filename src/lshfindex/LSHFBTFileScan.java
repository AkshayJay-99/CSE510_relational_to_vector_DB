 ...
 * class IndexFileScan.  
 */
public class LSHFBTFileScan  extends IndexFileScan
             implements  GlobalConst
{

  LSHFBTreeFile bfile; 
  String treeFilename;     // B+ tree we're scanning 
  LSHFBTLeafPage leafPage;   // leaf page containing current record
    ...
    try {
	 ...
	 if (nextpage.pid == INVALID_PAGE) {
	    ...
	    return null;
         ...
         leafPage=new LSHFBTLeafPage(nextpage, keyType);
	 	
	 entry=leafPage.getFirst(curRid);
      }

      if (endkey != null)  
        if ( LSHFBT.keyCompare(entry.key, endkey)  > 0) {
  ...
  */
  public  void DestroyLSHFBTreeFileScan()
