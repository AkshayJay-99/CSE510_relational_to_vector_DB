...
/**
 * A LSHFBTIndexPage is an index page on a B+ tree.  It holds abstract 
 ...
 */
public class LSHFBTIndexPage extends LSHFBTSortedPage{
 

  /** pin the page with pageno, and get the corresponding LSHFBTIndexPage,
   * also it sets the type of node to be NodeType.INDEX.
   *@param pageno Input parameter. To specify which page number the
   *  LSHFBTIndexPage will correspond to.
   *@param keyType either AttrType.attrInteger or AttrType.attrString.
   *   Input parameter.
   *@exception IOException error from the lower layer
   *@exception ConstructPageException error when LSHFBTIndexPage constructor
   */
  public LSHFBTIndexPage(PageId pageno, int keyType) 
    throws IOException, 
  ...
  /**associate the LSHFBTIndexPage instance with the Page instance,
   * also it sets the type of node to be NodeType.INDEX.
   ...
   *@exception ConstructPageException error when LSHFBTIndexPage constructor
   */
  public LSHFBTIndexPage(Page page, int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(page, keyType);
  ...
  /* new a page, associate the LSHFBTIndexPage instance with the Page instance,
   * also it sets the type of node to be NodeType.INDEX.
   *@param keyType either AttrType.attrInteger or AttrType.attrString.
   *  Input parameter.    
   *@exception IOException  error from the lower layer
   *@exception ConstructPageException error when LSHFBTIndexPage constructor
   */
  public LSHFBTIndexPage( int keyType) 
    throws IOException, 
	   ConstructPageException
    {
      super(keyType);
  ...
  RID deleteKey(KeyClass key) 
	...
	else if (LSHFBT.keyCompare(key, entry.key) != 0)
   ...
   * LSHFBTIndexPage by B++ search algorithm 
  ...
  PageId getPageNoByKey(KeyClass key) 
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  entry= LSHFBT.getEntryFromBytes( getpage(),getSlotOffset(i), 
   ...
   * which  provide an iterator interface to the records on a LSHFBTIndexPage.
  ...
  public KeyDataEntry getFirst(RID rid) 
	...
	entry=LSHFBT.getEntryFromBytes( getpage(),getSlotOffset(0), 
				    getSlotLength(0),
   ...
   * iterator interface to the records on a LSHFBTIndexPage.
  ...
  int  getSibling(KeyClass key, PageId pageNo)
	...
	for (i=getSlotCnt()-1; i >= 0; i--) {
	  ...
	  if (LSHFBT.keyCompare(key, entry.key)>=0) {
	    if (i != 0) { 
              entry=LSHFBT.getEntryFromBytes(getpage(), getSlotOffset(i-1),
   ...
   */
  boolean redistribute(LSHFBTIndexPage indexPage, LSHFBTIndexPage parentIndexPage,
	...
	if (direction==-1) { // 'this' is the left sibling of indexPage
	  ...
	  if ( (getSlotLength(0) + available_space()) > ((MAX_SPACE-DPFIXED)/2) ) {
            ...
            firstEntry=LSHFBT.getEntryFromBytes( getpage(),
					     getSlotOffset(0),
					     getSlotLength(0), keyType, NodeType.INDEX);
