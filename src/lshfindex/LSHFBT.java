 ...
 *
 */
public class LSHFBT  implements GlobalConst{
  ...
  protected final static int getKeyLength(KeyClass key) 
	    ...
	    IOException
    {
      ...
      //System.out.println("what type of key do we have? " + key);
  ...
  else if (keyType == AttrType.attrVector100D)
  {
      ...
      if (offset + expectedSize > from.length) {  // ✅ Check before reading
  ...
  public static void printPage(PageId pageno, int keyType)
    ...
    {
      LSHFBTSortedPage sortedPage=new LSHFBTSortedPage(pageno, keyType);
      int i;
      i=0;
      if ( sortedPage.getType()==NodeType.INDEX ) {
        LSHFBTIndexPage indexPage=new LSHFBTIndexPage((Page)sortedPage, keyType);
        System.out.println("");
	  ...
	  i++;    
        ...
        LSHFBTLeafPage leafPage=new LSHFBTLeafPage((Page)sortedPage, keyType);
        System.out.println("");
   ...
   */
  public static void printBTree(LSHFHeaderPage header) 
    throws IOException, 
	   ConstructPageException, 
	   IteratorException,
	   HashEntryNotFoundException,
  ...
  private static void _printTree(PageId currentPageId, String prefix, int i, 
	   ...
	   ReplacerException 
    {
      ...
      LSHFBTSortedPage sortedPage=new LSHFBTSortedPage(currentPageId, keyType);
      prefix=prefix+"       ";
      i++;
      if( sortedPage.getType()==NodeType.INDEX) {  
	 LSHFBTIndexPage indexPage=new  LSHFBTIndexPage((Page)sortedPage, keyType);
   ...
   *@exception ReplacerException  error from lower layer
   */
  public static void printAllLeafPages(LSHFHeaderPage header) 
    throws IOException, 
	   ConstructPageException, 
	   IteratorException,
	   HashEntryNotFoundException,
